...
# # from face_gui import face_authenticator
...
#     if "open file manager" in command or "open files" in command:
#         if os.name == 'nt':  # Windows
#             os.system('explorer')
#         elif os.name == 'posix':  # Mac/Linux
#             os.system('nautilus' if 'ubuntu' in os.uname().version.lower() else 'dolphin')
...
# #             st.text_area("Heard command:", value=command, height=50)
...
# #             execute_system_command("open file manager")
# #     with col2:
# #         if st.button("🌐 Open Browser"):
# #             execute_system_command("open browser")
# #     with col3:
# #         if st.button("💻 Open Terminal"):
# #             execute_system_command("open terminal")
...
def execute_system_command(command):
    ...
    try:
        ...
        if any(cmd in command for cmd in ["open file manager", "open files", "show files"]):
            if os.name == 'nt':  # Windows
                os.startfile(os.environ['USERPROFILE'])  # Opens user's home directory
        ...
        # System Control Commands
        elif any(cmd in command for cmd in ["shutdown", "shut down"]):
            response = "Shutdown command received (disabled for safety)"
            ...
            response = "Restart command received (disabled for safety)"
            # Uncomment to enable actual restart
            # if os.name == 'nt':  # Windows
...
def home_interface():
    ...
    with st.expander("💡 Available Commands"):
        ...
        - **System**: "take screenshot", "system info"
        """)
...
import webbrowser
from time import sleep
from face_gui import protect_resource  # Changed import

# Protected resources configuration (NEW)
PROTECTED_RESOURCES = {
    "Notepad": "notepad.exe",
...
def execute_system_command(command):
    ...
    try:
        ...
        # Protected Resources (MODIFIED)
        elif "open notepad" in command:
            protect_resource("Notepad", PROTECTED_RESOURCES["Notepad"])
            response = "Notepad access authorized"
        
        elif "open whatsapp" in command:
            protect_resource("WhatsApp", PROTECTED_RESOURCES["WhatsApp"])
            response = "WhatsApp access authorized"
        
        elif any(cmd in command for cmd in ["open documents", "my documents"]):
            protect_resource("Documents", PROTECTED_RESOURCES["Documents"])
            response = "Documents access authorized"
        
        elif "open project files" in command:
            protect_resource("Project Files", PROTECTED_RESOURCES["Project Files"])
            response = "Project files access authorized"
...
# ALL OTHER FUNCTIONS AND THE MAIN INTERFACE REMAIN EXACTLY THE SAME
# Only changed the execute_system_command() function above
...
# import webbrowser
# from time import sleep
# from face_gui import FaceAuthenticator  # Changed import
...
# # ALL OTHER CODE REMAINS EXACTLY THE SAME AS YOUR ORIGINAL main.py
# # Only changed the authentication-related parts above

# # ... [rest of your original code including home_interface() and main()] ...
# def home_interface():
#     """Enhanced home interface with voice and text input"""
#     st.title("🌟 Your AI Desktop Assistant")
#     st.write("Welcome to your personal assistant. How can I help you today?")
